...
@section('content')
    ...
    <h3>Записи: @if(isset($post)) редактирование @else добавление @endif записи</h3>
    <br>
    @if (count($errors) > 0)
    ...
    <form enctype="multipart/form-data" class="form-horizontal" action="{{ isset($post) ? route('admin.posts.update', ['id'=>@$post->id]) : route('admin.posts.store') }}" method="post">
            ...
            <div class="col-sm-7">
                <select name="category_id[]" class="form-control" multiple style="width: 200px; height: 100px">
                    @foreach($categories as $category)
                        <option value="{{$category->id}}" @if(in_array($category->id, old('category_id', @$post->categories->pluck('id')->toArray()))) selected @endif >{{$category->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-sm-3"></div>
